# MMdetection Multispectral v1.0.0

## Install (with docker)

```
git clone https://github.com/sejong-rcv/2022.mmdetection_multispectral.git
```

```
cd 2022.mmdetection_multispectral
```

Change the 'WORKDIR' in Dockerfile 

```
vim Dockerfile

WORKDIR mmdetection_path

(example) WORKDIR /home/jwkim/workspace/mmdetection
```
...
Run the container (Neet to change the name, volume, port and etc..)

```
(example) nvidia-docker run -it --name jwkim_mmdetection -v /data/jwkim/workspace:/workspace -v /data:/raid -p 8860:8860 -e NVIDIA_VISIBLE_DEVICES=all --shm-size=32G mmdetection /bin/bash
...
(example) train with multiple GPUs

```
./tools/dist_train.sh configs/multispectral/cascade_rcnn_hrnet_fpn.py 4
```

## Testing

(recommend) test with single GPU
```
python tools/test.py ${CONFIG_FILE} ${Checkpoint} ${Option}
```

(example) train with multiple GPUs
```
python tools/test.py configs/multispectral/cascade_rcnn_hrnet_fpn.py work_dirs/kaist-rgbt_cascade_rcnn_hrnetv2p_w32/latest.pth --eval miss-rate
```

## Visualization (for Multispectral)

```
python demo/ms_image_demo.py configs/multispectral/cascade_rcnn_hrnet_fpn.py work_dirs/kaist-rgbt_cascade_rcnn_hrnetv2p_w32/latest.pth
```

## Benchmarking 

### Benchmarking of multispectral pedestron model on KAIST pedestrian datasets 
### (Annotations : improved annotation, BMVC, 2018)

|    Detector                |  Dataset   | Backbone| Fusion Method  | Reasonable  | Heavy  |
|--------------------|:--------:|:--------:|:--------:|:--------:|:--------:|
| [Cascade Mask R-CNN Multispectral]() | kaist-rgbt      | HRNet | Halfway | 13.58 | 18.15 |
