    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.keep_ratio:
                ...
                img, scale_factor = mmcv.imrescale(results[key],results['scale'],return_scale=True,backend=self.backend)
                
                if 'lwir' in results.keys() :
                    lwir, scale_factor = mmcv.imrescale(results['lwir'],results['scale'],return_scale=True,backend=self.backend)
                ...
                img, w_scale, h_scale = mmcv.imresize(results[key],results['scale'],return_scale=True,backend=self.backend)
                
                if 'lwir' in results.keys() :
                    #import pdb;pdb.set_trace()
                    lwir, _, _ = mmcv.imresize(results['lwir'],results['scale'],return_scale=True,backend=self.backend)
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            for key in results.get('img_fields', ['img']):
                ...
                if 'lwir' in results.keys() :
                    results['lwir'] = mmcv.imflip(results['lwir'], direction=results['flip_direction'])
            ...
            for key in results.get('bbox_fields', []):
                ...
                # if 'lwir' in results.keys():
                #     results['lwir'] = self.bbox_flip(results['lwir'],
                #                               results['img_shape'],
                #                               results['flip_direction'])
            # flip masks
            for key in results.get('mask_fields', []):
                ...
                # if 'lwir' in results.keys():
                #     results['lwir'] = results['lwir'].flip(results['flip_direction'])
            ...
            for key in results.get('seg_fields', []):
                ...
                # if 'lwir' in results.keys():
                #     results['lwir'] = mmcv.imflip(
                #     results['lwir'], direction=results['flip_direction'])
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if self.size is not None:
                ...
                if 'lwir' in results.keys() :
                    padded_lwir = mmcv.impad(results['lwir'], shape=self.size, pad_val=pad_val)
                    
            elif self.size_divisor is not None:
                padded_img = mmcv.impad_to_multiple(results[key], self.size_divisor, pad_val=pad_val)
                
                if 'lwir' in results.keys() :
                    padded_lwir = mmcv.impad_to_multiple(results['lwir'], self.size_divisor, pad_val=pad_val)
                
            results[key] = padded_img
            if 'lwir' in results.keys() :
                results['lwir'] = padded_lwir
...
class Normalize:
    ...
    def __init__(self, mean, std, mean_lwir=None, std_lwir=None, to_rgb=True):
        self.mean = np.array(mean, dtype=np.float32)
        self.std = np.array(std, dtype=np.float32)
        
        if mean_lwir != None and std_lwir != None :
            self.mean_lwir = np.array(mean_lwir, dtype=np.float32)
            self.std_lwir = np.array(std_lwir, dtype=np.float32)
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if 'lwir' in results.keys() :
                results['lwir'] = mmcv.imnormalize(results['lwir'], self.mean_lwir, self.std_lwir, self.to_rgb)
...
class RandomCrop:
    ...
    def _crop_data(self, results, crop_size, allow_negative_crop):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if 'lwir' in results.keys() :
                lwir = lwir[crop_y1:crop_y2, crop_x1:crop_x2, ...]
...
class PhotoMetricDistortion:
    ...
    def __call__(self, results):
        ...
        # random brightness
        if random.randint(2):
            delta = random.uniform(self.brightness_delta - 0.5,
        ...
        if random.randint(2):
            ...
            if 'lwir' in results.keys() :
                lwir[..., 1] *= random.uniform(self.saturation_lower,
                                          self.saturation_upper)
        ...
        if random.randint(2):
            ...
            if 'lwir' in results.keys() :
                lwir[..., 0] += random.uniform(-self.hue_delta, self.hue_delta)
                lwir[..., 0][lwir[..., 0] > 360] -= 360
                lwir[..., 0][lwir[..., 0] < 0] += 360
        ...
        #     if 'lwir' in results.keys() :
        #         lwir = lwir[..., random.permutation(3)]
...
class Expand:
    ...
    def __call__(self, results):
        ...
        expand_thr = copy.deepcopy(expand_img)
        left = int(random.uniform(0, w * ratio - w))
        top = int(random.uniform(0, h * ratio - h))
        expand_img[top:top + h, left:left + w] = img

        if 'lwir' in results.keys():
            expand_thr[top:top + h, left:left + w] = lwir

        results['img'] = expand_img
        results['lwir'] = expand_thr
        ...
        for key in results.get('bbox_fields', []):
            ...
            if 'lwir' is results.keys():
                results['lwir'] = results['lwir'] + np.tile(
                (left, top), 2).astype(results['lwir'].dtype)
        ...
        for key in results.get('mask_fields', []):
            ...
            if 'lwir' is results.keys():
                results['lwir'] = results['lwir'].expand(
                int(h * ratio), int(w * ratio), top, left)
    ...
    def __call__(self, results):
        ...
        while True:
            ...
            for i in range(50):
                ...
                if 'lwir' in results.keys() :
                    lwir = lwir[patch[1]:patch[3], patch[0]:patch[2]]
                results['img'] = img
                ...
                for key in results.get('seg_fields', []):
                    results[key] = results[key][patch[1]:patch[3],
                                                patch[0]:patch[2]]
                    if 'lwir' in results.keys():
                        results['lwir'] = results['lwir'][patch[1]:patch[3],
                                                patch[0]:patch[2]]
...
class RandomCenterCropPad:
    ...
    def _train_aug(self, results):
        ...
        while True:
            ...
            for i in range(50):
                ...
                if 'lwir' in results.keys():
                    lwir_cropped_img, border, patch = self._crop_image_and_paste(
                        lwir, [center_y, center_x], [new_h, new_w])
    ...
    def _test_aug(self, results):
        ...
        if 'lwir' in results.keys() :
            lwir_cropped_img, border, _ = self._crop_image_and_paste(
            lwir, [h // 2, w // 2], [target_h, target_w])
