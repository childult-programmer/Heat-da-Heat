...
class MS_LoadImageFromFile:
    """Load an Multispectral images from file.
    ...
    def __call__(self, results):
        ...
        if results['img_prefix'] is not None:
            ...
            filename_lwir = osp.join(results['img_prefix'],results['img_info']['filename'].replace("visible","lwir"))
        else:
            filename = results['img_info']['filename']
            filename_lwir = results['img_info']['filename'].replace("visible","lwir")
        
        img_bytes = self.file_client.get(filename)
        lwir_bytes = self.file_client.get(filename_lwir)
        img = mmcv.imfrombytes(img_bytes, flag=self.color_type)
        lwir = mmcv.imfrombytes(lwir_bytes, flag=self.color_type)
        

        if img.shape != (512, 640, 3):
            print("rgb")
            print(filename)
            print(filename_lwir)
            import pdb;pdb.set_trace()
        if lwir.shape != (512, 640, 3):
            print("thr")
            print(filename)
            print(filename_lwir)
            import pdb;pdb.set_trace()
