...
class HF_SSDVGG(VGG, BaseModule):
    ...
    def __init__(self,
                 ...
                 ceil_mode=True,
                 out_indices=(3, 4),
                 out_feature_indices=(22, 22),
                 pretrained=None,
                 init_cfg=None,
                 input_size=None,
                 l2_norm_scale=None):
        # TODO: in_channels for mmcv.VGG
        super(HF_SSDVGG, self).__init__(
            depth,
        ...
        self.hf_out_feature_indices = (34, 34)
    ...
    def forward(self, x):
        ...
        #import pdb;pdb.set_trace()
        for i, layer in enumerate(self.features[:23]):
            #import pdb;pdb.set_trace()
            vis = layer(vis)
            if i in self.out_feature_indices:
                vis_outs.append(vis)

        for i, layer in enumerate(self.features_lwir[:23]):
            lwir = layer(lwir)
            if i in self.out_feature_indices:
                lwir_outs.append(lwir) 

        #import pdb;pdb.set_trace()

        hf_outs = []

        #Concat rgb_conv4_3, thermal_conv4_3
        outs = [x + y for x, y in zip(vis_outs, lwir_outs)]

        # Maxpool concat feature map
        outs = self.features[23](outs[0])

        # Append concat conv4_3
        hf_outs.append(outs)
        
        #import pdb;pdb.set_trace()
        
        for i, layer in enumerate(self.features[24:]): 
            outs = layer(outs)
            if (i+24) in self.hf_out_feature_indices:
                #import pdb;pdb.set_trace()
                hf_outs.append(outs)
